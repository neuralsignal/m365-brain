...
m365-extract provides two service-specific Dockerfiles and a single `docker-compose.yaml` for local development.

## Docker Compose

The `docker-compose.yaml` runs the full stack locally: PostgreSQL, the Reflex admin UI (`Dockerfile.web`), and the sync daemon (`Dockerfile.daemon`). An Azurite blob emulator is available via the `azurite` profile.
