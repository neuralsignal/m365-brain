...
class TestPerChatIsolation:
    ...
    def test_transport_error_on_messages_fetch_skips_chat(self, httpx_mock: HTTPXMock, storage, client):
        """A TransportError during the messages fetch (not media) must skip the chat."""
        _mock_chats(httpx_mock)
        httpx_mock.add_exception(
            httpx.ConnectError("network down"), url=re.compile(r".*/me/chats/.*/messages.*"), is_reusable=True
        )
...
class TestRelations:
    def test_relations_section_rendered_for_long_participant_names(self, httpx_mock: HTTPXMock, storage, client):
        """Participants with slugified names longer than 5 chars produce Relations links."""
        httpx_mock.add_response(
            url=re.compile(r".*/me/chats\?.*"),
            json={
                "value": [
