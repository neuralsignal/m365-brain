...
def _conv_dir(paths: VaultPaths) -> str:
    """One chat directory, resolved through the layout config the helpers read."""
    return paths.inbox_item("teams_chats", "foo_abc")
...
class TestDownloadMessageAttachments:
    def test_reference_attachment_written(self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths) -> None:
        ...
        files = storage.list_files(vault_paths.inbox_root("teams_chats"))
        assert vault_paths.attachment(_conv_dir(vault_paths), "msg-1", "spec.pdf") in files
        client.close()

    def test_skipped_reference_types_emit_no_request(
    ...
    def test_oversized_attachment_skipped(self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths) -> None:
        ...
        assert refs == []
        assert storage.list_files(vault_paths.inbox_root("teams_chats")) == []
        client.close()
    ...
    def test_convert_called_for_matching_extension(
        ...
        assert target_path == vault_paths.converted_attachment(_conv_dir(vault_paths), "msg-7", "spec.pdf.md")
        client.close()

    def test_failed_conversion_clears_converted_path(
        self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths
    ) -> None:
    ...
    def test_path_traversal_in_name_stripped(self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths) -> None:
        ...
        files = storage.list_files(vault_paths.inbox_root("teams_chats"))
        assert vault_paths.attachment(_conv_dir(vault_paths), "msg-8", "escape.txt") in files
...
class TestPermanentFailureSkipList:
    ...
    def test_nonfile_reference_types_silently_skipped(
        self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths
    ) -> None:
        storage = LocalBackend(str(tmp_path / "vault"))
        client = GraphClient(graph_config, lambda: "test-token")
...
class TestDownloadInlineImages:
    def test_inline_image_written_and_mapped(self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths) -> None:
        chat_id = "19:abc"
        msg_id = "1"
        hid = "HID-1"
        httpx_mock.add_response(
        ...
        hosted_map = hosted_content.download_inline_images(
            _ctx(client, storage, vault_paths, settings=_config()), f"/chats/{chat_id}/messages/{msg_id}", msg
        )

        assert hosted_map == {hid: f"attachments/{msg_id}/inline_0.png"}
        files = storage.list_files(vault_paths.inbox_root("teams_chats"))
        assert vault_paths.attachment(_conv_dir(vault_paths), msg_id, "inline_0.png") in files
        client.close()

    def test_unknown_content_type_falls_back_to_bin(
        self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths
    ) -> None:
        chat_id = "19:def"
        msg_id = "2"
    ...
    def test_oversized_hosted_content_skipped(self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths) -> None:
        chat_id = "19:ghi"
        msg_id = "3"
        hid = "HID-3"
        httpx_mock.add_response(
        ...
        hosted_map = hosted_content.download_inline_images(
            _ctx(client, storage, vault_paths, settings=_config(max_mb=1)),
    ...
    def test_fetch_failure_returns_empty(self, httpx_mock: HTTPXMock, tmp_path, graph_config, vault_paths) -> None:
        chat_id = "19:jkl"
        msg_id = "4"
        httpx_mock.add_response(
            url=re.compile(rf".*/chats/{re.escape(chat_id)}/messages/{msg_id}/hostedContents.*"),
    ...
    def test_download_transport_error_skipped(self, tmp_path, graph_config, vault_paths) -> None:
        ...
        hosted_map = hosted_content.download_inline_images(
            _ctx(mock_client, storage, vault_paths, settings=_config()),
            f"/chats/{chat_id}/messages/{msg_id}",
            {"id": msg_id},
        )
    ...
    def test_empty_hosted_content_id_skipped(self, tmp_path, graph_config, vault_paths) -> None:
        from unittest.mock import MagicMock
    ...
    def test_storage_error_on_write_skips_item(self, tmp_path, graph_config, vault_paths) -> None:
        from unittest.mock import MagicMock
        ...
        hosted_map = hosted_content.download_inline_images(
            _ctx(mock_client, mock_storage, vault_paths, settings=_config()),
    ...
    def test_os_error_on_write_skips_item(self, tmp_path, graph_config, vault_paths) -> None:
        from unittest.mock import MagicMock
...
class TestFraudulentDomainSSRF:
    def test_blocked_domain_raises(self, tmp_path, graph_config, vault_paths) -> None:
...
class TestResolveAttachment:
    ...
    def test_transport_error_returns_none(self, tmp_path, graph_config, vault_paths) -> None:
        from unittest.mock import MagicMock
