...
class TestBuildStoragePathProperty:
    ...
    def test_output_starts_with_prefix_ends_with_md_contains_hash(self, prefix, file_name, item_id):
        from m365_brain.m365.markdown_writer import short_hash
...
class TestRemovalHandler:
    """`handle_removed_item` moved verbatim into `RemovalHandler.remove`; same assertions."""

    def test_deletes_tracked_file(self, tmp_path, ctx):
        storage = LocalBackend(str(tmp_path / "vault"))
        storage.write_file("onedrive/test.md", "content")

        file_paths = {"item-1": "onedrive/test.md"}
        ctx.removal.remove(extractor="onedrive", upstream_id="item-1", path_map=file_paths)

        assert "item-1" not in file_paths
        assert not storage.file_exists("onedrive/test.md")

    def test_ignores_untracked_item(self, tmp_path, ctx):
        file_paths = {}
        ctx.removal.remove(extractor="onedrive", upstream_id="unknown-id", path_map=file_paths)
        assert len(file_paths) == 0


class TestProcessDriveItem:
    def test_stub_for_non_eager_convertible(self, tmp_path, ctx):
        storage = LocalBackend(str(tmp_path / "vault"))
        item = {
            "name": "doc.docx",
            "size": 1024,
    ...
    def test_stub_for_non_convertible(self, tmp_path, ctx):
        storage = LocalBackend(str(tmp_path / "vault"))
        item = {
            "name": "image.png",
            "size": 5000,
    ...
    def test_eager_convert_downloads_and_converts(self, tmp_path, ctx):
        storage = LocalBackend(str(tmp_path / "vault"))
    ...
    def test_download_failure_writes_error_stub(self, tmp_path, ctx):
        storage = LocalBackend(str(tmp_path / "vault"))
        mock_client = MagicMock()
...
class TestProcessDriveItemFallbackFetch:
    def test_fetches_download_url_when_missing_from_delta(self, tmp_path, ctx):
    ...
    def test_handles_graph_api_error_during_individual_fetch(self, tmp_path, ctx):
...
class TestProcessDriveItemConversionError:
    def test_writes_error_stub_on_conversion_failure(self, tmp_path, ctx):
...
class TestGraphApiErrorNotSwallowed:
    def test_download_graph_api_error_produces_error_stub(self, tmp_path, ctx):
    ...
    def test_refetch_graph_api_error_produces_no_url_stub(self, tmp_path, ctx):
...
class TestFileProcessingContextImmutable:
    def test_is_frozen(self, ctx):
        file_ctx = FileProcessingContext(
            client=MagicMock(),
            storage=MagicMock(),
            file_config=FileProcessingConfig(
                eager_patterns=[],
        ...
        with pytest.raises(FrozenInstanceError):
            file_ctx.storage = MagicMock()  # type: ignore[misc]
