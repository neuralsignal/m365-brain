...
class TestEmptyHostedContentId:
    def test_empty_id_is_skipped(self, vault_paths):
        client = _make_client([{"id": ""}, {"id": "hc-valid"}])
        storage = MagicMock()
        msg = {"id": "m1"}

        result = download_inline_images(
            _ctx(client, storage, vault_paths, settings=_settings()), "/chats/c1/messages/m1", msg
        )
    ...
    def test_missing_id_key_is_skipped(self, vault_paths):
...
class TestStorageWriteFailure:
    def test_storage_error_skips_item_and_continues(self, vault_paths):
    ...
    def test_os_error_skips_item_and_continues(self, vault_paths):
        client = _make_client([{"id": "hc1"}, {"id": "hc2"}])
        storage = MagicMock()
        storage.write_bytes.side_effect = [
    ...
    def test_storage_error_logs_warning(self, caplog, vault_paths):
