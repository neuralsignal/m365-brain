...
class TestDownloadableAttachmentNames:
    ...
    def test_matches_download_message_attachments_result(self, tmp_path, vault_paths):
        ...
        refs = download_message_attachments(
            _ctx(client, storage, "conv", vault_paths, settings=settings, converters_config={}, failed_attachments={}),
            msg,
        )
...
class TestToStoredMessageReuse:
    def _convert(self, msg: dict, prior: StoredMessage | None, client, paths: VaultPaths) -> StoredMessage:
        settings = _settings(download_attachments=True, download_inline_images=True)
        storage = MagicMock()
        return to_stored_message(
            _ctx(client, storage, "conv", paths, settings=settings, converters_config={}, failed_attachments={}),
            msg,
            None,
    ...
    def test_reaction_only_bump_reuses_prior_media_without_client_calls(self, vault_paths):
        ...
        result = self._convert(msg, prior, client, vault_paths)
    ...
    def test_edited_message_does_not_reuse_prior_content(self, vault_paths):
    ...
    def test_tombstone_does_not_reuse_prior_media(self, vault_paths):
        prior_refs = [{"name": "spec.pdf", "relative_path": "attachments/m1/spec.pdf", "converted_path": None}]
    ...
    def test_changed_attachment_set_does_not_reuse(self, vault_paths):
        prior_refs = [{"name": "spec.pdf", "relative_path": "attachments/m1/spec.pdf", "converted_path": None}]
        prior = _stored("m1", etag="1", content="hello", attachments=prior_refs, deleted=False)
...
class TestTeamsContextImmutable:
    ...
    def test_is_frozen(self, vault_paths):
        ctx = _ctx(
            MagicMock(),
            MagicMock(),
            "conv",
    ...
    def test_failed_attachments_is_shared_not_copied(self, vault_paths):
