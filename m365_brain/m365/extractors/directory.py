...
def run(
    ...
    # No `$filter=accountEnabled eq true`, deliberately, and this is the one
    # behavioural change in an otherwise mechanical relocation. Server-side
    # filtering made a disabled account *vanish from the results* rather than
    # arrive flagged, so the flip was undetectable by construction and the
    ...
    written = 0
    for user in users:
        # A hard-deleted user arrives as @removed, with no displayName --
        # _should_skip_user would drop it as invalid and the page would
        # persist. Disabled and deleted are different signals for the same
        # outcome, and both have to be read.
        if "@removed" in user:
            ctx.removal.remove(extractor=name, upstream_id=user.get("id", ""), path_map=path_map)
            continue

        # `is False`, not falsy: only an explicit flag deletes. An absent
        # field must never be read as "disabled" -- that would empty the
        # vault on a $select change.
        if config.only_active_users and user.get("accountEnabled") is False:
            ctx.removal.remove(extractor=name, upstream_id=user.get("id", ""), path_map=path_map)
            continue
        ...
        if _write_user(storage, user_data, ctx, path_map):
            written += 1
