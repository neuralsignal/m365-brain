"""CLI entry point for m365-brain.

Commands:
  m365-brain auth login       — Authenticate via device code flow
  m365-brain auth status      — Show cached token info
  m365-brain sync --once      — Run all enabled extractors once
  m365-brain sync --dry-run   — Validate auth + scopes without writing files
  m365-brain worker           — Run the sync worker (multi-user, per-extractor jobs)
"""
...
def main(ctx: click.Context, config_path: str) -> None:
    """m365-brain: Sync Microsoft 365 data to Obsidian-compatible markdown."""
...
def status(ctx: click.Context) -> None:
    ...
    if not cache_path.exists():
        click.echo("No cached token found. Run: m365-brain --config config.yaml auth login")
        raise SystemExit(1)
...
def worker(ctx: click.Context) -> None:
    ...
    from m365_brain.worker import worker_loop
